...
class User implements UserInterface, PasswordAuthenticatedUserInterface
{
    ...
    /**
     * @ORM\OneToMany(targetEntity=HelloLike::class, mappedBy="user")
     */
    private $likes;

    /**
     * @ORM\OneToMany(targetEntity=ByeLikes::class, mappedBy="user")
     */
    private $byeLikes;
    ...
    public function removeByeLike(ByeLikes $byeLike): self
    {
        if ($this->byeLikes->removeElement($byeLike)) {
            // set the owning side to null (unless already changed)
            if ($byeLike->getUser() === $this) {
                $byeLike->setUser(null);
            }
        }
